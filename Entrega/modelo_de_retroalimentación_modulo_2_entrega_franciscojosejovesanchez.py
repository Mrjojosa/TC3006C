...
df = pd.DataFrame(ds)
df2 = df[['sugars','fat']].copy()

#Tabla y gráfica
print(df2)
plt.scatter(ds['sugars'], ds['fat'], marker='o', c='b')
...
epochs = 80
# Entrena el modelo
w, b = train(df2['sugars'], df2['fat'], w=0.0, b=0.0, alpha=0.001, epochs=80)
...
for epoch_plt in epoch_plots:
  w, b = train_and_plot(df2['sugars'], df2['fat'], 0.0, 0.0, 0.001, epoch_plt, int(max(df['sugars'])))
